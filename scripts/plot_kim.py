"""Visualize the Kim-filter smoother output for one patient.

Produces:
  data/plots/<pid>_kim.png               -- static 2-panel diagnostic
  data/plots/<pid>_kim_interactive.html  -- interactive (zoom/pan) plotly

Usage (from asthma-prediction/):
    python scripts/plot_kim.py 0010
"""
import sys
...
df = pd.read_csv(Path(f"data/smoothed_kim/{PID}_kim.csv"))
df["createdTime"] = pd.to_datetime(df["createdTime"])
OUT_DIR = Path("data/plots"); OUT_DIR.mkdir(parents=True, exist_ok=True)
COL = "#e377c2"   # Kim pink (distinct from the other methods)
...
ax1.plot(df["createdTime"], df["smoothed_hrv"], c=COL, lw=0.6, alpha=0.8,
         label="Kim-filter smoothed")
ax1.set_title(f"Patient {PID} — Markov-switching Kim filter (full series, "
...
ax2.plot(zwin["createdTime"], zwin["smoothed_hrv"], c=COL, lw=1.8,
         label="Kim-filter smoothed", zorder=3)
ax2.set_title("Zoom: densest 3-day window — 2-regime switching (Hamilton + Kalman, causal)")
...
for ax in (ax1, ax2):
    ax.grid(alpha=0.25)
fig.tight_layout()
fig.savefig(OUT_DIR / f"{PID}_kim.png", dpi=130)
plt.close(fig)
print(f"static  -> {OUT_DIR / f'{PID}_kim.png'}")
...
fig.add_trace(go.Scattergl(
    x=df["createdTime"], y=df["smoothed_hrv"], mode="lines", name="Kim-filter smoothed",
    line=dict(color=COL, width=1.3), connectgaps=False))
fig.update_layout(
    title=f"Patient {PID} — Markov-switching Kim filter (drag to zoom, use range slider). "
          f"Line breaks at ≥180-min gaps.",
    xaxis_title="time", yaxis_title="HRV (SDNN, ms)",
    template="plotly_white", hovermode="x unified",
    legend=dict(orientation="h", y=1.06, x=0))
fig.update_xaxes(rangeslider_visible=True)
fig.write_html(OUT_DIR / f"{PID}_kim_interactive.html", include_plotlyjs="cdn")
print(f"interactive -> {OUT_DIR / f'{PID}_kim_interactive.html'}")
