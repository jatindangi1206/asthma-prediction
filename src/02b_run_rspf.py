"""Stage 2b (alternative smoother): Regime-Switching Particle Filter.

A NEW, independent smoothing method for benchmarking against the existing
`particles` SMC+FFBS smoother (`02_run_filters.py`). This script does NOT modify
or read the existing `data/smoothed/` outputs.

  Input :  data/processed/<pid>_processed.csv   (from 00_preprocess_raw.py)
  Output:  data/smoothed_rspf/<pid>_rspf.csv

Output schema mirrors `data/smoothed/` so downstream stages can run unchanged
against this directory:

  createdTime, hrvValue, minute_diff, smoothed_hrv, true_trend_level,
  gap_flag, chunk_id

  * smoothed_hrv     = RS-PF causal filtered estimate (full signal, > 0)
  * true_trend_level = RS-PF circadian-removed baseline (the trend level, > 0)
  * chunk_id         = H8 segment id (resets at >= 180-min gaps)

To benchmark CPD on this method:  point 03_annotate.py's INPUT_DIR at
`data/smoothed_rspf` (it auto-uses true_trend_level, falling back to smoothed_hrv).

Run from the asthma-prediction/ directory:
    python src/02b_run_rspf.py
"""
...
# Import the core smoother. Works whether run as `python src/02b_run_rspf.py`
# or `python -m src.02b_run_rspf`.
try:
    from rspf_smoother import smooth_dataframe, RSPFConfig
...
OUTPUT_DIR = Path("./data/smoothed_rspf")
RESULTS_DIR = Path("./data/results")

# ---- Method configuration (the K you want lives here) --------------------
# K = number of latent regimes (H2/H4). 2 = awake/asleep, 3 = asleep/rest/active.
GLOBAL_K = 3
GLOBAL_N_PARTICLES = 500


def process_patient(args):
    """Smooth one processed patient CSV with the RS-PF and write the output."""
    file_path, output_dir, k, n_particles = args
    try:
        ...
        # and let the filter smooth across a 4-hour gap. The H8 rule is defined
        # on consecutive *readings*, so we segment on observed rows only: drop
        # the fillers, let smooth_dataframe compute time_diff between real
        # readings, then merge the smoothed values back onto the full grid.
        obs = df[df["hrvValue"].notna()].copy()
        obs["patient_id"] = stem

        cfg = RSPFConfig(K=k, n_particles=n_particles)
        ...
        res = df.copy()
        merge_cols = ["createdTime", "smoothed_hrv", "true_trend_level", "chunk_id"]
        ...
        out_cols = ["createdTime", "hrvValue", "minute_diff", "smoothed_hrv",
                    "true_trend_level", "residual_hrv", "gap_flag", "chunk_id"]
        Path(output_dir).mkdir(parents=True, exist_ok=True)
        res[out_cols].to_csv(Path(output_dir) / f"{stem}_rspf.csv", index=False)
...
def run_rspf_smoothing(k=GLOBAL_K, n_particles=GLOBAL_N_PARTICLES):
    ...
    print(f"Regimes K={k}  Particles={n_particles}  Gap threshold=180 min")

    max_workers = max(1, multiprocessing.cpu_count() - 1)
    print(f"Workers:    {max_workers}\n")

    args_list = [(f, OUTPUT_DIR, k, n_particles) for f in csv_files]
    results = []

    with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers) as executor:
        futures = {executor.submit(process_patient, a): a for a in args_list}
        for future in tqdm(concurrent.futures.as_completed(futures),
                           total=len(args_list), desc="RS-PF smoothing"):
            results.append(future.result())

    df_log = pd.DataFrame(results)
    log_path = RESULTS_DIR / "smoothing_rspf_log.csv"
    ...
    print("\n" + "=" * 60)
    print("RS-PF SMOOTHING COMPLETE")
    print("=" * 60)
