"""Stage 2i (alternative smoother): H-RKF (Huber Robust Kalman Filter, log-domain).
...
(`02d`), OSSA (`02e`), Kim (`02f`), Gamma DGLM (`02g`) and Log-MHW (`02h`). It
does NOT modify or read their outputs.

  Input :  data/processed/<pid>_processed.csv   (from 00_preprocess_raw.py)
  Output:  data/smoothed_hrkf/<pid>_hrkf.csv

Output columns:
  createdTime, hrvValue, minute_diff, smoothed_hrv, gap_flag, chunk_id

  * smoothed_hrv = exp(F'm_t) from the causal log-domain Huber robust KF (> 0)
  * chunk_id     = H8 segment id (resets at >= 180-min gaps)

Run from the asthma-prediction/ directory:
    python src/02i_run_hrkf.py
"""
...
try:
    from hrkf_smoother import smooth_dataframe, HRKFConfig
...
OUTPUT_DIR = Path("./data/smoothed_hrkf")
...
GLOBAL_HUBER_DELTA = 1.345  # Huber threshold in std units (smaller => more robust)


def process_patient(args):
    file_path, output_dir, deltrend, delseas, huber_delta = args
    try:
        ...
        cfg = HRKFConfig(deltrend=deltrend, delseas=delseas, huber_delta=huber_delta)
        ...
        res[out_cols].to_csv(Path(output_dir) / f"{stem}_hrkf.csv", index=False)
...
def run_hrkf_smoothing(deltrend=GLOBAL_DELTREND, delseas=GLOBAL_DELSEAS,
                       huber_delta=GLOBAL_HUBER_DELTA):
    ...
    print("\n--- H-RKF SMOOTHER (Huber robust Kalman filter, log-domain) ---")
    print(f"Input:      {INPUT_DIR.resolve()}")
    print(f"Output:     {OUTPUT_DIR.resolve()}")
    print(f"Patients:   {len(csv_files)}")
    print(f"deltrend={deltrend}  delseas={delseas}  huber_delta={huber_delta}  Gap=180 min")

    max_workers = max(1, multiprocessing.cpu_count() - 1)
    print(f"Workers:    {max_workers}\n")

    args_list = [(f, OUTPUT_DIR, deltrend, delseas, huber_delta) for f in csv_files]
    results = []

    with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers) as executor:
        futures = {executor.submit(process_patient, a): a for a in args_list}
        for future in tqdm(concurrent.futures.as_completed(futures),
                           total=len(args_list), desc="H-RKF smoothing"):
            results.append(future.result())

    df_log = pd.DataFrame(results)
    log_path = RESULTS_DIR / "smoothing_hrkf_log.csv"
    ...
    print("\n" + "=" * 60)
    print("H-RKF SMOOTHING COMPLETE")
    print("=" * 60)
