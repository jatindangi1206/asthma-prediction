"""Stage 2d (alternative smoother): Gaussian-Process State-Space Model.

A NEW, independent smoothing method for benchmarking against the existing
SMC+FFBS particle filter (`02_run_filters.py`), the RS-PF (`02b_run_rspf.py`)
and KRLST (`02c_run_krlst.py`). It does NOT modify or read their outputs.

  Input :  data/processed/<pid>_processed.csv   (from 00_preprocess_raw.py)
  Output:  data/smoothed_gpssm/<pid>_gpssm.csv

Output columns:
  createdTime, hrvValue, minute_diff, smoothed_hrv, gap_flag, chunk_id

  * smoothed_hrv = GP-SSM causal Kalman-filtered estimate (single output, > 0)
  * chunk_id     = H8 segment id (resets at >= 180-min gaps)

Run from the asthma-prediction/ directory:
    python src/02d_run_gpssm.py
"""
...
try:
    from gpssm_smoother import smooth_dataframe, GPSSMConfig
...
OUTPUT_DIR = Path("./data/smoothed_gpssm")
RESULTS_DIR = Path("./data/results")

# ---- Method configuration ------------------------------------------------
GLOBAL_Q_SCALE = 0.05    # process-noise multiplier (smaller => smoother)
GLOBAL_R_SCALE = 5.0     # obs-noise multiplier    (larger  => smoother)


def process_patient(args):
    """Smooth one processed patient CSV with the GP-SSM and write the output."""
    ...
    try:
        ...
        # chunker. The H8 rule is defined on consecutive *readings*, so segment
        # on observed rows only, then merge results back onto the full grid.
        obs = df[df["hrvValue"].notna()].copy()
        obs["patient_id"] = stem

        cfg = GPSSMConfig(q_scale=q_scale, r_scale=r_scale)
        ...
        res[out_cols].to_csv(Path(output_dir) / f"{stem}_gpssm.csv", index=False)
...
def run_gpssm_smoothing(q_scale=GLOBAL_Q_SCALE, r_scale=GLOBAL_R_SCALE):
    ...
    print("\n--- GP-SSM SMOOTHER (GP-learned dynamics + causal Kalman filter) ---")
    print(f"Input:      {INPUT_DIR.resolve()}")
    print(f"Output:     {OUTPUT_DIR.resolve()}")
    print(f"Patients:   {len(csv_files)}")
    print(f"q_scale={q_scale}  r_scale={r_scale}  Gap threshold=180 min")
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in tqdm(concurrent.futures.as_completed(futures),
                           total=len(args_list), desc="GP-SSM smoothing"):
            results.append(future.result())

    df_log = pd.DataFrame(results)
    log_path = RESULTS_DIR / "smoothing_gpssm_log.csv"
    ...
    print("\n" + "=" * 60)
    print("GP-SSM SMOOTHING COMPLETE")
    print("=" * 60)
