"""Stage 2k (alternative smoother): CT-IMM (Continuous-Time Interacting Multiple Model).
...
(`02d`), OSSA (`02e`), Kim (`02f`), Gamma DGLM (`02g`), Log-MHW (`02h`), H-RKF
(`02i`) and AN-REWMA (`02j`). It does NOT modify or read their outputs.

  Input :  data/processed/<pid>_processed.csv   (from 00_preprocess_raw.py)
  Output:  data/smoothed_ctimm/<pid>_ctimm.csv

Output columns:
  createdTime, hrvValue, minute_diff, smoothed_hrv, residual_hrv, gap_flag, chunk_id

Run from the asthma-prediction/ directory:
    python src/02k_run_ctimm.py
"""
...
try:
    from ct_imm_smoother import smooth_dataframe, CTIMMConfig
...
OUTPUT_DIR = Path("./data/smoothed_ctimm")
RESULTS_DIR = Path("./data/results")

# ---- Method configuration ------------------------------------------------
GLOBAL_Q_SCALE = 0.001   # base process noise (× R); smaller => smoother
GLOBAL_STICKINESS = 0.95 # mode self-transition probability


def process_patient(args):
    file_path, output_dir, q_scale, stickiness = args
    try:
        ...
        cfg = CTIMMConfig(q_scale=q_scale, stickiness=stickiness)
        ...
        res[out_cols].to_csv(Path(output_dir) / f"{stem}_ctimm.csv", index=False)
...
def run_ctimm_smoothing(q_scale=GLOBAL_Q_SCALE, stickiness=GLOBAL_STICKINESS):
    ...
    print("\n--- CT-IMM SMOOTHER (continuous-time interacting multiple model) ---")
    print(f"Input:      {INPUT_DIR.resolve()}")
    print(f"Output:     {OUTPUT_DIR.resolve()}")
    print(f"Patients:   {len(csv_files)}")
    print(f"q_scale={q_scale}  stickiness={stickiness}  modes=3  Gap threshold=180 min")

    max_workers = max(1, multiprocessing.cpu_count() - 1)
    print(f"Workers:    {max_workers}\n")

    args_list = [(f, OUTPUT_DIR, q_scale, stickiness) for f in csv_files]
    results = []

    with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers) as executor:
        futures = {executor.submit(process_patient, a): a for a in args_list}
        for future in tqdm(concurrent.futures.as_completed(futures),
                           total=len(args_list), desc="CT-IMM smoothing"):
            results.append(future.result())

    df_log = pd.DataFrame(results)
    log_path = RESULTS_DIR / "smoothing_ctimm_log.csv"
    ...
    print("\n" + "=" * 60)
    print("CT-IMM SMOOTHING COMPLETE")
    print("=" * 60)
