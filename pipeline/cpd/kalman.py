...
_R = np.array([[10.0]])
_Q_FACTOR = 0.1
_THRESHOLD = 10.0  # Lower = more adverse flags
_NORMAL_THRESHOLD = 2.5  # Threshold between normal and medium
...
def detect(
    ...
    normal_th = _NORMAL_THRESHOLD

    if os.environ.get("HPO_ACTIVE") != "1":
        config_path = Path(__file__).parent / "kalman_params.json"
        if config_path.exists():
            try:
                with open(config_path, "r") as f:
                    config = json.load(f)
                threshold = float(config.get("threshold", threshold))
                normal_th = float(config.get("normal_th", normal_th))
                print(f"[Kalman] Loaded optimized parameters: threshold={threshold:.2f}, normal_th={normal_th:.2f}")
            except Exception as e:
                print(f"[Kalman] Warning: Failed to load config from {config_path}: {e}")


    change_types = np.empty(N, dtype=object)
    change_types[scores <= normal_th] = "normal"  # Lower = more medium/adverse flags
    change_types[(scores > normal_th) & (scores <= threshold)] = "medium"
    change_types[scores > threshold] = "adverse"

    change_degrees = np.minimum(scores / threshold, 1.0)
